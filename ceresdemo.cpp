...
public:
    ...
    // 添加Eigen::Vector3<T> base_sensor_offset;
	// Eigen::Quaternion<T> base_sensor_rot_offset;
	template <typename T> bool operator()(const T* const p_global_gain, const T* const p_sensor_offset, const T* const p_sensor_rot_offset, 
		...
		const T* const p_crosstalk,const T* const p_base_sensor_offset, const T* const p_base_sensor_rot_offset, T* residual) const
	{
		...
		// 添加base_sensor_offset base_sensor_rot_offset
		Eigen::Map<const Eigen::Vector3<T>> base_sensor_offset(p_base_sensor_offset);
		Eigen::Map<const Eigen::Quaternion<T>> base_sensor_rot_offset(p_base_sensor_rot_offset);
...
void solve_global_model(const vector<DataFrame>& rawData, CoilCalibrationModel<double>& coil_param, int sensor_offset)
{
	...
	ceres::CostFunction* cost_function_global = new ceres::AutoDiffCostFunction<GlobalCostFunctor, 3*N, 1, 3, 4, 3, 4, 3, 3, 3, 3, 3,3,4>(cost);
	// 添加base_sensor_offset base_sensor_rot_offset
	global_problem.AddResidualBlock(cost_function_global, NULL, &coil_param.global_gain, coil_param.sensor_offset.data(), coil_param.sensor_rot_offset.coeffs().data(),
		...
		coil_param.crosstalk.data(),coil_param.base_sensor_offset.data(), coil_param.base_sensor_rot_offset.coeffs().data());

	// ############################print coil paramaters ##################################
	// cout<<coil_param.noise.data()<<endl;
	// ####################################################################################
