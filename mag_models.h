...
{
	T global_gain;//初始化了
	Eigen::Vector3<T> noise;//初始化了
	Eigen::Vector3<T> bias;//初始化了
	Eigen::Vector3<T> gain;//初始化了
	Eigen::Vector3<T> per_channel_gain;//初始化了
	Eigen::Vector3<T> sensor_offset; //初始化了
	Eigen::Quaternion<T> sensor_rot_offset;//初始化了
	Eigen::Vector3<T> base_sensor_offset;
	Eigen::Quaternion<T> base_sensor_rot_offset;
	Eigen::Vector3<T> ring_offset;//初始化了
	Eigen::Quaternion<T> ring_rot_offset;//初始化了
	array<T, 3> crosstalk;//初始化了
...
struct CalibrationModel
{
	...
	// template<typename U> CalibrationModel<U> cast() const
	// {
	// 	CalibrationModel<U> model;
	// 	model.coil1 = coil1.cast<U>();
	// 	model.coil2 = coil2.cast<U>();
	// 	model.coil3 = coil3.cast<U>();
	// 	return model;
	// }
};
